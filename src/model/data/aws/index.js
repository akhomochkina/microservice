...
async function deleteFragment(ownerId, id) {
  ...
  const DBcommand = new DeleteCommand(other);

  try {
    await s3Client.send(command);
  } catch (err) {
    const { Bucket, Key } = params;
    logger.error({ err, Bucket, Key }, 'Unable to delete');
  ...
  try {
    ...
    const { TableName, Key } = other;
    logger.error({ err, TableName, Key }, 'Unable to delete metadata');
